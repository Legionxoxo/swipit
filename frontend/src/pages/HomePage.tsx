...
export function HomePage() {
    ...
    const unifiedCreators: UnifiedCreator[] = useMemo(() => {
        ...
        const deduplicatedCreators = Array.from(creatorMap.values());
        
        console.debug('HomePage unified creators debug:', {
            youtubeCount: youtube.length,
            instagramCount: instagram.length,
            youtubeIds: youtube.map(c => c.analysisId),
            instagramIds: instagram.map(c => c.analysisId),
            totalUnified: youtube.length + instagram.length,
            deduplicatedCount: deduplicatedCreators.length,
            deduplicatedIds: deduplicatedCreators.map(c => c.analysisId)
        });
        
        return deduplicatedCreators;
